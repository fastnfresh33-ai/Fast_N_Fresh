...
| Production | `https://your-deployed-api-domain.com/api` |
...
### Using your own signing key for a real release

Right now `android/app/build.gradle` signs release builds with the debug key so
the project builds out of the box. Before publishing for real:
...
2. Create `android/key.properties` (never commit this file):
   ...
   ```
3. Update the `signingConfigs.release` block in `android/app/build.gradle` to read
   from `key.properties` instead of falling back to the debug key.
