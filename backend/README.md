...
npm install
cp .env.example .env
# edit .env if needed (JWT_SECRET, ports, seed credentials, etc.)
```
...
See `.env.example` for the full list. Never commit a real `.env` with production secrets.
...
4. Add environment variables from `.env.example` in the dashboard (use a MongoDB Atlas URI
